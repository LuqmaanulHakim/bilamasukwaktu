...
import { useWaktuSolatWeek, PrayerWeek } from "../hooks/useWaktuSolatWeek";
...
export default function PrayerTimesGrid({ zone }: Props) {
  ...
  const handleNext = () => setIndex((i) => Math.min(week.length - 1, i + 1));
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div>
          <p className="text-xs font-medium uppercase tracking-widest" style={{ color: "var(--muted)" }}>
            {index === 0 ? "Today" : displayed?.day ?? ""}
          </p>
          <p className="text-base font-semibold mt-0.5" style={{ color: "var(--foreground)" }}>
            {displayed?.date ?? "--"}
          </p>
        </div>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            style={{
              background: isDark ? "rgba(51,65,85,0.5)" : "rgba(0,0,0,0.06)",
              color: index === 0
