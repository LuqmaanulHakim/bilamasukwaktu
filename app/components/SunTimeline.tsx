...
export default function SunTimeline({ zone }: SunTimelineProps) {
  ...
  function degToX(deg: number) { return padX + ((deg + 90) / 360) * visW; }
  function degToY(deg: number) {
    const s = Math.sin((deg * Math.PI) / 180);
    return axisY - (s > 0 ? s * amp : s * dipAmp);
  }
  function hourToDeg(hour: number): number {
    if (hour <= sunrise)     return -90 + (hour / sunrise) * 90;
    else if (hour <= sunset) return ((hour - sunrise) / (sunset - sunrise)) * 180;
    else                     return 180 + ((hour - sunset) / (24 - sunset)) * 90;
  }
  ...
  const prayerDotStroke = "var(--accent)";
  const sunColor        = isDaytime ? (isDark ? "#fbbf24" : "#f59e0b") : (isDark ? "#475569" : "#94a3b8");
  const sunGlow         = isDaytime ? (isDark ? "rgba(251,191,36,0.5)" : "rgba(245,158,11,0.5)") : "rgba(100,116,139,0.3)";
  ...
  return (
    ...
    >
      ...
      {/* SVG graph */}
      <div className="w-full flex justify-center">
        <svg width="100%" viewBox={`0 0 ${svgW} ${svgH}`} style={{ overflow: "visible", maxWidth: svgW }}>
          <path d={leftDipPath}  fill={belowFill} />
          <path d={rightDipPath} fill={belowFill} />
          <line x1={xMinus90} y1={axisY} x2={x270} y2={axisY} stroke={axisColor} strokeWidth="1" />
          <path d={trackPath}  fill="none" stroke={trackColor}  strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          {activePoints.length > 1 && (
            <path d={activePath} fill="none" stroke={activeColor} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          )}

          {/* Syuruk */}
          <g>
            <circle cx={sunriseDot.px} cy={sunriseDot.py} r={5} fill={sunriseDot.isPast ? prayerDotStroke : prayerDotFill} stroke={prayerDotStroke} strokeWidth="1.5" />
            <text x={sunriseDot.px + getLabelOffsetX("Syuruk")} y={sunriseDot.py + 16} textAnchor="middle" fontSize="8" fontFamily="inherit" fill={labelColor}>Syuruk</text>
          </g>

          {/* Maghrib */}
          <g>
            <circle cx={sunsetDot.px} cy={sunsetDot.py} r={5} fill={sunsetDot.isPast ? prayerDotStroke : prayerDotFill} stroke={prayerDotStroke} strokeWidth="1.5" />
            <text x={sunsetDot.px + getLabelOffsetX("Maghrib")} y={sunsetDot.py + 16} textAnchor="middle" fontSize="8" fontFamily="inherit" fill={labelColor}>Maghrib</text>
          </g>

          {/* Prayer dots */}
          {prayerDots.map(({ name, px, py, labelX, labelY, labelPos, isPast }) => (
            <g key={name}>
              <line x1={px} y1={py + (labelPos === "above" ? 5 : -5)} x2={px} y2={axisY} stroke={leaderColor} strokeWidth="1" strokeDasharray="2 2" />
              <circle cx={px} cy={py} r={5} fill={isPast ? prayerDotStroke : prayerDotFill} stroke={prayerDotStroke} strokeWidth="1.5" />
              <text x={labelX} y={labelY} textAnchor="middle" fontSize="8" fontFamily="inherit" fill={labelColor}>{name}</text>
            </g>
          ))}

          {/* Sun */}
          <circle cx={sunX} cy={sunY} r={8} fill={sunColor} style={{ filter: `drop-shadow(0 0 5px ${sunGlow})` }} />
        </svg>
      </div>
